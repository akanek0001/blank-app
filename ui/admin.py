...
class AdminPage:
    ...
    @classmethod
    def _safe_df(cls, df: pd.DataFrame | None) -> pd.DataFrame:
        if df is None:
            return pd.DataFrame()
    ...
    @classmethod
    def _render_html_table(cls, df: pd.DataFrame | None) -> None:
        ...
        st.markdown(
            f'<div class="admin-table-wrap">{html_table}</div>',
            unsafe_allow_html=True,
        )
    ...
    def render(
        ...
        if not line_users_df.empty:
            tmp = (
                line_users_df[
                    line_users_df["Line_User_ID"].astype(str).str.startswith("U")
                ]
                .drop_duplicates(subset=["Line_User_ID"], keep="last")
                .copy()
            )
        ...
        if not view_all.empty:
            ...
            cand = (
                ...
                else view_all[view_all["IsActive"] == True].copy().reset_index(drop=True)
            )
        ...
        if add_mode == "個人(PERSONAL)":
            ...
            project_candidates = [
                p for p in all_projects if str(p).strip().upper() != AppConfig.PROJECT["PERSONAL"]
            ]
