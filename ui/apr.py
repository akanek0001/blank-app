...
TX_DATE_BOTTOM_OFFSET_RATIO_MOBILE = 70 / 2532

TX_TYPE_LEFT_RATIO_MOBILE = 100 / 1170
TX_TYPE_RIGHT_RATIO_MOBILE = 760 / 1170
TX_TYPE_TOP_OFFSET_RATIO_MOBILE = 38 / 2532
TX_TYPE_BOTTOM_OFFSET_RATIO_MOBILE = 125 / 2532

TX_USD_LEFT_RATIO_MOBILE = 760 / 1170
TX_USD_RIGHT_RATIO_MOBILE = 1090 / 1170
TX_USD_TOP_OFFSET_RATIO_MOBILE = 28 / 2532
TX_USD_BOTTOM_OFFSET_RATIO_MOBILE = 92 / 2532
    ...
    def _build_region_box(self, row_top: float, left_ratio: float, right_ratio: float, top_offset_ratio: float, bottom_offset_ratio: float) -> Dict[str, float]:
        return {"left": left_ratio, "top": row_top + top_offset_ratio, "right": right_ratio, "bottom": row_top + bottom_offset_ratio}

    def _extract_date_label(self, text: str) -> str:
        t = self._normalize_ocr_text(text)
        for pat in [r"(\d{1,2}\s*月\s*\d{1,2}\s*日)", r"(\d{1,2}\s*月\s*\d{1,2})", r"(\d{1,2}/\d{1,2})", r"(\d{1,2}-\d{1,2})"]:
            m = re.search(pat, t)
            if m:
    ...
    def _ocr_transaction_rows_three_regions(self, file_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        for i in range(self.TX_SCAN_MAX_ROWS_MOBILE):
            ...
            date_box = self._build_region_box(row_top, self.TX_DATE_LEFT_RATIO_MOBILE, self.TX_DATE_RIGHT_RATIO_MOBILE, self.TX_DATE_TOP_OFFSET_RATIO_MOBILE, self.TX_DATE_BOTTOM_OFFSET_RATIO_MOBILE)
            type_box = self._build_region_box(row_top, self.TX_TYPE_LEFT_RATIO_MOBILE, self.TX_TYPE_RIGHT_RATIO_MOBILE, self.TX_TYPE_TOP_OFFSET_RATIO_MOBILE, self.TX_TYPE_BOTTOM_OFFSET_RATIO_MOBILE)
            usd_box = self._build_region_box(row_top, self.TX_USD_LEFT_RATIO_MOBILE, self.TX_USD_RIGHT_RATIO_MOBILE, self.TX_USD_TOP_OFFSET_RATIO_MOBILE, self.TX_USD_BOTTOM_OFFSET_RATIO_MOBILE)
    ...
    def _process_transaction_ocr_three_regions(self, file_bytes: bytes, source_image: str, source_project: str) -> None:
        ...
        for row in tx_rows:
            ...
            if is_duplicate:
                ...
                new_sheet_rows.append([unique_key, row["date_label"], row["time_label"], row["type_label"], amount_usd, "", "", source_image, source_project, row["raw_text"], created_at])
            view_rows.append({"Row": row["row_index"], "Date_Label": row["date_label"], "Time_Label": row["time_label"], "Type_Label": row["type_label"], "Amount_USD": f"{amount_usd:.2f}", "Unique_Key": unique_key, "Status": "重複" if is_duplicate else "新規"})
        if new_sheet_rows:
    ...
    def render(self, settings_df: pd.DataFrame, members_df: pd.DataFrame) -> None:
        ...
        folder_path = st.text_input("監視フォルダパス", value=default_watch_folder, key="apr_watch_folder", placeholder="/Users/yourname/Desktop/smartvault_images")
...
""")
        ...
        with c_tx1:
            if st.button("現在の画像から3領域OCR集計", use_container_width=True, key="ocr_tx_current_three"):
                if selected_evidence_bytes:
                    self._process_transaction_ocr_three_regions(selected_evidence_bytes, selected_evidence_name or "selected_image", project)
                else:
        ...
        for p in target_projects:
            ...
            for _, r in mem_calc.iterrows():
                ...
                preview_rows.append({"Project_Name": p, "PersonName": person, "Rank": str(r["Rank"]).strip(), "Compound_Timing": U.compound_label(compound_timing), "Principal": U.fmt_usd(float(r["Principal"])), "DailyAPR": U.fmt_usd(float(r["DailyAPR"])), "Line_User_ID": str(r["Line_User_ID"]).strip(), "LINE_DisplayName": str(r["LINE_DisplayName"]).strip(), "流動性": U.fmt_usd(float(total_liquidity)), "昨日の収益": U.fmt_usd(float(yesterday_profit)), "APR": f"{apr:.4f}%", "本日APR状態": "本日記録済み" if is_done else "未記録"})
